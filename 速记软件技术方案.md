...
│  │  分类生成Agent │ 标签提取Agent │ 将来事项识别Agent │   │
│  │  关系发现Agent │ 事件关联Agent │ 语义搜索Agent     │   │
│  └──────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────┐   │
│  │         Redis 组件 (延迟队列 + 广播通知)           │   │
│  │  DelayQueue │ Broadcast │ FutureEventStorage      │   │
│  └──────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────┐   │
│  │         Whisper 语音识别服务                       │   │
│  └──────────────────────────────────────────────────┘   │
...
用途三：延迟队列（DelayQueue）
  delay_queue:reminders -> Sorted Set         将来事项提醒队列
  score=执行时间戳，value=任务数据JSON

用途四：广播通知（Broadcast）
  agent:find_relations:future_events          Agent间通信频道
  user:{user_id}:reminders                    用户提醒频道

用途五：将来事项存储（FutureEventStorage）
  future_events:user:{user_id} -> Hash        将来事项数据
  field=memo:{memo_id}, value=事件数据JSON     TTL=30天
...
└─────────────────────────────────────────────────────────────┘
                    ...
                    └────────┬────────┘
                             ...
                             ▼
                    ┌─────────────────┐
                    │     extract     │  提取标签、实体、时间信息
                    │  (提取节点)      │  (time_type, datetime_str)
                    └────────┬────────┘
                             │
                             ▼
                    ┌─────────────────┐
                    │ future_reminder │  识别将来事项，设置提醒
                    │  (将来事项提醒)  │  (延迟队列 + 广播通知)
                    └────────┬────────┘
                             │
                             ▼
                    ┌─────────────────┐
                    │ find_relations  │  查找关联内容（含将来事项）
                    │  (关系发现节点)  │  (速记关联速记/事件)
                    └────────┬────────┘
                             │
                    ┌────────┴────────┐
...
class MemoProcessState(TypedDict):
    ...
    tags: list[str]
    entities: list[dict]
    time_info: dict  # 新增：时间信息
    reminder_result: dict  # 新增：提醒结果
    related_items: list[dict]

def load_context_node(state: MemoProcessState) -> MemoProcessState:
    """节点1: 加载用户图谱上下文"""
    ...
...
def future_reminder_node(state: MemoProcessState) -> MemoProcessState:
    """节点4: 将来事项提醒 — 识别将来事项并设置提醒"""
    # 提取时间信息
    # 如果是将来时间且需要提醒：
    #   - 解析时间字符串（支持多种格式）
    #   - 添加到Redis延迟队列
...
def find_relations_node(state: MemoProcessState) -> MemoProcessState:
    """节点5: 查找关联内容（速记关联速记，事件关联事件/速记）"""
    # 查询用户的将来事项
    # 基于标签和实体匹配关联
    ...

def judge_relations_node(state: MemoProcessState) -> MemoProcessState:
    """节点6: 判定关联关系（仅速记）"""
    ...

def bind_events_node(state: MemoProcessState) -> MemoProcessState:
    """节点7: 绑定事件（仅速记）"""
    ...

def persist_graph_node(state: MemoProcessState) -> MemoProcessState:
    """节点8: 将所有结果写入 Neo4j"""
    ...
...
workflow.add_conditional_edges(
    "find_relations",
    lambda state: "event_path" if state["memo_type"] == "event" else "quicknote_path",
    {
...
3. LangGraph Agent 执行：
   ...
   - **将来事项识别**：
     - 如果识别到将来时间且需要提醒：
       - 解析时间字符串（支持多种格式：ISO、相对时间、自然语言）
       - 添加到 Redis 延迟队列（DelayQueue）
       - 广播通知关系发现 Agent（Broadcast）
       - 存储到 Redis 将来事项表（FutureEventStorage）
   - 用提取的实体和标签去 Neo4j 查询相关内容（包括将来事项）
...
将来事项识别Agent是速记处理流程中的关键节点，负责识别用户速记中的将来事项并设置提醒。

**功能特性：**

1. **时间信息提取**
   - 从速记内容中提取时间信息（time_type, datetime_str, is_reminder, reminder_type）
   - 支持多种时间格式：
     - ISO格式：`2026-02-20T14:30:00`
     - 简单格式：`2026-02-20 14:30:00`
     - 相对时间：`3天后`、`2小时后`、`30分钟后`
     - 自然语言：`明天`、`后天`、`下周一`、`下周三`

2. **提醒类型识别**
   - `deadline`：截止日期（如"下周五之前完成"）
   - `appointment`：约会/会议（如"明天下午2点开会"）
   - `task`：任务/待办事项（如"下周要完成项目文档"）
...
4. **广播通知**
   - 通过Redis Pub/Sub广播通知关系发现Agent
   - 通知用户提醒频道（`user:{user_id}:reminders`）
   - 支持实时推送提醒消息

5. **将来事项存储**
   - 存储到Redis Hash（`future_events:user:{user_id}`）
   - 包含完整的提醒信息（标题、内容、时间、类型等）
   - 自动过期（30天）
...
└─────────────────────────────────────────────────────────────┘
        ...
        ▼                     ▼                     ▼
┌───────────────┐    ┌───────────────┐    ┌───────────────┐
│  DelayQueue   │    │   Broadcast   │    │FutureEvent    │
│  (延迟队列)    │    │  (广播通知)    │    │  Storage      │
└───────────────┘    └───────────────┘    └───────────────┘
        │                     │                     │
        │                     │                     │
        ▼                     ▼                     ▼
┌───────────────┐    ┌───────────────┐    ┌───────────────┐
│Sorted Set     │    │Pub/Sub        │    │Hash           │
│delay_queue:   │    │agent:find_    │    │future_events: │
│reminders      │    │relations:     │    │user:{user_id} │
│               │    │future_events  │    │               │
│score=时间戳   │    │user:{id}:     │    │field=memo:{id}│
└───────────────┘    │reminders      │    └───────────────┘
                     └───────────────┘
```
...
future_reminder节点处理
    │
    ├── 解析时间字符串
    │   └── parse_datetime("下周三") → 2026-02-18
    │
    ├── 计算延迟秒数
    │   └── delay_seconds = (reminder_time - now).total_seconds()
    │
    ├── 添加到延迟队列
    │   └── DelayQueue.push(task_data, delay_seconds)
    │
    ├── 存储将来事项
    │   └── FutureEventStorage.store_future_event()
    │
    └── 广播通知
        ├── Broadcast.publish("agent:find_relations:future_events")
        └── Broadcast.publish("user:{user_id}:reminders")
    │
    ▼
find_relations节点查询
    │
...
延迟队列工作线程
    │
    ├── 启动worker
    │   └── DelayQueue.start_worker(handler)
    │
    ├── 轮询到期任务
    │   └── DelayQueue.pop()
    │
...
async def future_reminder_node(state: MemoProcessState) -> dict:
    ...
    # 检查是否是将来时间且需要提醒
    if time_info.get("time_type") != "future" or not time_info.get("is_reminder"):
        return {"reminder_result": {"has_reminder": False}}
    
    # 解析时间
    reminder_time = parse_datetime(time_info.get("datetime_str"))
    delay_seconds = int((reminder_time - datetime.now()).total_seconds())
...
将来事项识别Agent通过Redis广播通知关系发现Agent，关系发现Agent在查找关联时会查询用户的将来事项，基于标签和实体进行匹配，实现将来事项与现有速记的自动关联。
